...
import { Mail, Shield, Palette, Zap, Bell as BellIcon, Send } from 'lucide-react';
      ...
      try {
        ...
        if (userSettings) {
          setSettings({
            ...
            reportSchedule: userSettings.reportSchedule || 'weekly',
            tradeReminders: userSettings.emailSettings?.tradeReminders ?? true,
            maxDailyLoss: userSettings.riskSettings?.maxDailyLoss ?? 5,
          });
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      const result = await updateUserSettings(user.uid, {
        ...
        emailSettings: { tradeReminders: settings.tradeReminders },
        riskSettings: { maxDailyLoss: Number(settings.maxDailyLoss) }
      });
      if (result.success) {
